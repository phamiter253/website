<script setup>
  ...
  const showRatings = () => {
    ...
    svg.selectAll("rect.rating")
      ...
      .attr("y", function(d, i){
        if (i > 0){
          y6.value = y6.value + (candy[i-1].dimensions[1]) + 10
        }
        const curr = y6.value + d.wrapOffset/2
        return curr;
      })
      ...
      .ease(d3.easeCubicOut)
      .attr("width", function(d){
        return d.rating/100 * (d.dimensions[0] - (d.offset ? 5 : 200));
      })
  ...
  const renderChart = () => {
    ...
    gradient.append("stop")
      .attr("offset", "0%")
      .attr("stop-color", "#caa906");

    gradient.append("stop")
      .attr("offset", "25%")
      .attr("stop-color", "#e1c16e");
    ...
    const ytransform = selectedType.value === 'sugar' ? 778 : 8
    defs.append("clipPath")
      .attr("id", 'reeses-clip')
      .append("path")
      .attr('d', reesePath)
      .attr('transform', `translate(100, ${ytransform})`)
  }
